...
class Course extends MY_Controller {
	...
	function update_pricing(){
		// check if exist
		$check = $this->Course_m->get_course_pricing(array(
							'cp.course_id' => $this->input->post('cid', true),
							'cpm.program_id' => $this->input->post('prog', true),
							'cp.city_id' => $this->input->post('city', true)
						));
		// print_r($this->db->last_query());
		// if not exist then add
		if($check==false){
			$add_data = array(
				'course_id' => $this->input->post('cid', true),
				'city_id' => $this->input->post('city', true),
				'price_per_hour' => $this->input->post('price-per-hour', true),
			...
			$upd = $this->Common->update_data_on_table('course_pricing', 'id', $check->row()->id, $upd_data);
		...
		if(empty($this->any_error))
			...
			$response = array(
				'status' => '204',
				'message' => $this->any_error[0]
				);

		echo json_encode($response);
	}

	function duplicate_pricing(){
		$response = array();
		// 1. make sure you have data in source table
		$src_data = $this->Course_m->get_course_pricing(array('cp.city_id' => $this->input->get('csrc', true)));
		if($src_data<>false){
			// 2. remove data where city and course id exist
			$del_old = $this->Common->delete_from_table_by_id('course_pricing', 'city_id', $this->input->get('ctrg', true));
			// print_r($this->db->last_query());
			// 2. insert data
			if($del_old->status){	
				foreach($src_data->result() as $src){
					$add_data = array(
						'course_id' => $src->course_id,
						'city_id' => $this->input->get('ctrg', true),
						'price_per_hour' => $src->price_per_hour,
						'general_price' => $src->general_price,
						'module_price' => $src->module_price,
						'tryout_price' => $src->tryout_price
						);
					$add = $this->Common->add_to_table('course_pricing', $add_data);
					$this->push_if_transaction_error($add);
				}
			...
			$response = array(
				'status' => '204',
				'message' => 'There is no data in source, make sure the source data is exist.'
				);
		}
	...
	function get_pricings(){
		...
		// get category data
		$get_data = $this->Course_m->get_course_pricing(array(
							'cpm.program_id' => $this->input->get('prog_id', true),
							'cp.city_id' => $this->input->get('city', true)
						));
		$response = array();
		if($get_data<>false){
		...
		// jika ada data course di kota x yang belum diisi, akan ditampilkan
		$more_data = $this->Course_m->get_course_pricing_not_filled($this->input->get('city', true), $this->input->get('prog_id', true));
		if($more_data<>false){
			foreach($more_data->result() as $data_m)
				array_push($response, array(
					'id' => $data_m->cid,
					'course_code' => $data_m->course_code,
					'course_name' => $data_m->course_name,
					'price_per_hour' => '',
					'general_price' => '',
					'module_price' => '',
					'tryout_price' => ''
