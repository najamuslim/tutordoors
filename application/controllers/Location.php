...
class Location extends MY_Controller {
	...
	function update_province(){
		...
		if($upd)
			...
			$response = array('status'=>'301', 'message' => $upd->output);
	...
	function add_city(){
		$city_name = strtolower($this->input->post('city', TRUE));
		$slug_temp = preg_replace('/[^A-Za-z0-9\-]/', '-', $city_name);
		$check_slug = $this->Location_m->check_exist_slug('cities', $slug_temp);
		
		if($check_slug <> false){
			$last_char = is_numeric($check_slug) ? intval($check_slug) : 0;
			$slug = $slug_temp.'-'.str_pad(strval($last_char + 1), 2, "0", STR_PAD_LEFT);
		}
		else
			$slug = $slug_temp;
		$data = array(
	...
	function get_queryable_cities_by_province($prov_id){
		$cities = $this->Location_m->get_city(array('c.province_id' => $prov_id, 'c.queryable' => "1"));
		if($cities==false){
			$response = array('status' => '204');
		}
		else{
	...
	function set_province_queryable(){
		$data = array('queryable' => $this->input->post('val', TRUE));
		$upd = $this->Common->update_data_on_table('provinces', 'province_id', $this->input->post('id', TRUE), $data);
		if($upd)
			$response = array('status'=>'200');
		else
