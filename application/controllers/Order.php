...
class Order extends MY_Controller {
	...
	function add_cart(){
		$this->order_lib->clear_cart_session();
		if($this->session->userdata('level')=="teacher"){
			$response['status'] = '301';
			$response['message'] = $this->lang->line('error_teacher_cannot_enter');
		}
			...
			else
			{
				...
				foreach($this->input->post('course') as $selected_course){
			        ...
			        $pricings = $this->Course_m->get_course_pricing(array('cp.course_id' => $course_info->row()->id, 'cp.city_id' => $this->input->post('city', true) ));
		...
		if($any_error==false)
		{
			...
			foreach($cart_array as $teacher_id => $course_data){
	            ...
	            foreach($course_data as $arr){
                    ...
                    $fee_per_hour = $arr['pph'];
                    // calculate sub price
		            $sub_price = $fee_per_hour * $arr['session_hour'] * $arr['class_in_month'] / 1; // unit jam ngajar yang berlaku = 1 jam
		            ...
		            $data = array(
						...
						'teacher_salary_per_hour' => $fee_per_hour,
	...
	function preorder_set_city($city_id){
		$city_array = $this->order_lib->get_selected_city_session();
		if($trans=="add"){
