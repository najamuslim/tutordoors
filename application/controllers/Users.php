...
class Users extends MY_Controller {
	...
	public function user_add(){
		...
		{
			$check_exist_email = $this->User_m->check_user_email_exist($this->input->post('email', TRUE));
	...
	public function do_login(){
		...
		if(!$user_data)
			...
			if($current_page=="cms/login"){
				if($user_data->user_level=="admin" or $user_data->user_level=="admin_staff" or $user_data->user_level=="academic_staff" or 
                                    $user_data->user_level=="finance_staff" or $user_data->user_level=="content_staff"){
					$response['status'] = '200';
	...
	function add_education_history(){
		...
		$data = array(
			...
			'is_graduated' => $this->input->post('is-graduated', true) == "" ? "0" : "1"
			);
	...
	function update_education_history(){
		...
		$data = array(
			...
			'major' => $this->input->post('major', true),
			'date_in' => $this->input->post('year_in', true),
			'date_out' => $this->input->post('year_out', true) == "" ? "0" : $this->input->post('year_out', true),
			'grade_score' => $this->input->post('grade_score', true),
			'is_graduated' => $this->input->post('year_out', true) == "" ? "0" : "1"
			);
	...
	public function check_exist_email(){
		$user_info = $this->User_m->check_user_email_exist($this->input->get('e', TRUE));
		if($user_info==false)
			$response = array(
				'status' => '204',
