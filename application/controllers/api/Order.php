...
class Order extends REST_Controller {
	...
	function save_post(){
      ...
      //       $salary = $this->User_m->get_salary_per_hour($teacher_id);
            // get modul price & tryout
            $pricings = $this->Course_m->get_course_pricing(array('cp.course_id' => $this->post('course_id'), 'cp.city_id' => $this->post('city_id') ));
			if($pricings <> false)
	        	$price_per_hour = $pricings->row()->price_per_hour;

            // calculate sub price
            $sub_price = $price_per_hour * $this->post('session') * $this->post('class') / 1; // unit jam ngajar yang berlaku = 1 jam
            ...
            $data = array(
				...
				'module_price' => '0',
				'tryout_price' => '0',
				'teacher_salary_per_hour' => $price_per_hour,
	...
	function get_course_pph_get(){
		$response = array();

		$course_id = $this->get('coid');
		$city_id = $this->get('ciid');

		$pricings = $this->Course_m->get_course_pricing(array('cp.course_id' => $course_id, 'cp.city_id' => $city_id ));
		if($pricings <> false)
        	$price_per_hour = $pricings->row()->price_per_hour;
        else
        	$price_per_hour = '0';

        $response = array('pph' => $price_per_hour);
