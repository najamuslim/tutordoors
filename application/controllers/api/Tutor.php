...
class Tutor extends REST_Controller {
	...
	function add_education_post(){
		...
		else
		{
			...
			$data = array(
				...
				'is_graduated' => $this->post('is_graduated') == "" ? "0" : "1"
				);
	...
	function update_education_post(){
		// 1. validate form
		$this->load->library('form_validation');
		...
		$this->form_validation->set_rules('year_in', $this->lang->line('college_year_in'), 'required|numeric');
		$this->form_validation->set_rules('year_out', $this->lang->line('college_year_out'), 'required|numeric');
		$this->form_validation->set_rules('eid', "Edu ID", 'numeric');
		...
		else
		{
			...
			$add_edu = $this->Common->update_data_on_table('user_education_experiences', 'id', $this->post('eid'), $data);
	...
	function delete_education_post(){
		...
		$this->form_validation->set_rules('eid', "Edu ID", 'required|numeric');
		...
		{	
			$add_edu = $this->Common->delete_from_table_by_id('user_education_experiences', 'id', $this->post('eid'));
	...
	function profile_get(){
		...
		if($user_info->verified_user=="0")
			...
			$get_latest_edu = $this->User_m->get_education_history_by_userid($user_id);
            $latest_edu = $get_latest_edu<>false ? $get_latest_edu->row()->degree.' '.$get_latest_edu->row()->major.' - '.$get_latest_edu->row()->institution : '';
			$response['latest_education'] = $latest_edu;
	...
	function personal_get(){
		...
		$response = array(
			...
			'education' => $edu_data <> false ? $edu_data->result() : array()
			);
		$this->response($response);
	}

	function education_user_get(){
	...
	function education_id_get(){
		$edu_id = $this->get('eid');
		$edu_data = $this->User_m->get_education_history_by_id($edu_id);
		$response = array(
			'education' => $edu_data <> false ? $edu_data : array()
			);
		$this->response($response);
	}
	...
	function update_personal_education_post(){
		...
		else
		{
			...
			// check if user exist in user_info_data
			$check = $this->User_m->get_user_info_data($this->post('tid'));
			if($check <> false){
				$upd_info = $this->Common->update_data_on_table('user_info_data', 'user_id', $this->post('tid'), $personal_data);
				if(!$upd_info->status)
	...
	function remove_open_course_get(){
		$id = $this->get('id');
	...
	function dashboard_get(){
		...
		// 3. check photo profile & phone
		$user_info = $this->User_m->get_user_info($user_id);
		$response['photo_set'] = "0";
		$response['profile_set'] = "0";
		if($user_info<>false){
			$response['photo_set'] = $user_info->file_name == "" ? "0" : "1";
			$response['profile_set'] = $user_info->phone_1 == "" ? "0" : "1";
		}

		// get open city fo course
        $check_city = $this->Teacher_m->get_open_city('oc.user_id', $user_id);

        // get open course
        $check_course = $this->Teacher_m->get_course_data_by_userid($user_id);

        $response['city_set'] = $check_city == false ? "0" : "1";
        $response['course_set'] = $check_course == false ? "0" : "1";

		// get the latest version
		$this->load->model('Software_m');
		$get_version = $this->Software_m->get_latest_version('android-td-id');
		$response['latest_version'] = $get_version->version;
