...
class Course_m extends CI_Model {
  ...
  //   }
	...
	function get_course_pricing_not_filled($city_id, $program_id){
		$query = $this->db->query("SELECT cs.id cid, cs.course_code, cs.course_name, cs.slug
			FROM course_programs cpm
			JOIN courses cs ON cpm.program_id = cs.program_id
			where cpm.program_id = '".$program_id."' and cs.id not in (
			SELECT cs.id
			FROM course_programs cpm
			JOIN courses cs ON cpm.program_id = cs.program_id
			JOIN course_pricing cp ON cs.id = cp.course_id
			WHERE cpm.program_id = '".$program_id."' and cp.city_id = '".$city_id."'
			)");
