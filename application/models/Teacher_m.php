...
class Teacher_m extends CI_Model {
    ...
    function get_verified_open_course_by_teacher_id($teacher_id, $concat=TRUE){
    	if($concat==TRUE)
    		...
    		$this->db->select('toc.id as open_course_id, cs.id as course_id, cs.course_name, cp.program_name, toc.fee_per_hour');
    ...
    function get_data_concat_course($filter_array){
		$this->db->select("toc.user_id, toc.days, toc.session_hours, GROUP_CONCAT(cs.course_name SEPARATOR ', ') AS courses, cp.program_name, pr.province_name, ci.city_name, u.first_name, u.last_name, u.user_level,
		...
		$this->db->join('provinces pr', 'ci.province_id = pr.province_id');
	...
	function get_fee_per_hour($tutor_id, $course_id){
		$this->db->select('fee_per_hour');
		$this->db->from('teacher_open_courses');
	...
	function get_unverified(){
		$this->db->select('u.user_id, u.first_name, u.last_name, ui.sex, u.email_login, u.join_date, ui.salary_per_hour, ui.phone_1, 
		ui.phone_2, ui.birth_place, ui.birth_date, ui.national_card_number, ui.address_national_card, ui.address_domicile,
		ui.occupation, ui.about_me, ui.teach_experience, ui.certification, ui.skill, ui.toefl_score, ui.hobby, ui.where_student_school,
		ui.religion, ui.rejected_reason, uee.grade_score,
		m.file_name');
		$this->db->from('users u');
		$this->db->join('user_info_data ui', 'u.user_id = ui.user_id', 'left');
		$this->db->join('user_education_experiences uee', 'u.user_id = uee.user_id', 'left');
		$this->db->join('media_files m', 'ui.photo_primary_id = m.id', 'left');
	...
	function get_verified(){
		$this->db->select('u.user_id, u.email_login, u.first_name, u.last_name, u.join_date, ui.sex, ui.phone_1, m.file_name, ui.salary_per_hour');
		$this->db->from('users u'); 
		$this->db->join('user_info_data ui', 'u.user_id = ui.user_id', 'left');
		$this->db->join('media_files m', 'ui.photo_primary_id = m.id', 'left');
		$this->db->where('u.verified_user', '1');
	...
	function get_total_tutor_in_province($is_verified_user){
		$this->db->select('c.province_id, c.province_name, count(*) as total', false)
					->from('teacher_open_course_cities a')
					->join('cities b', 'a.city_id = b.city_id')
					->join('provinces c', 'b.province_id = c.province_id')
					->join('users d', 'a.user_id = d.user_id')
					->where('d.verified_user', $is_verified_user)
					->group_by('c.province_id, c.province_name')
					->order_by('c.province_id, c.province_name');
	...
	function get_total_tutor_in_city($province_id, $is_verified_user){
		$this->db->select('b.city_id, b.city_name, count(*) as total', false)
					->from('teacher_open_course_cities a')
					->join('cities b', 'a.city_id = b.city_id')
					->join('users d', 'a.user_id = d.user_id')
					->where('b.province_id', $province_id)
					->where('d.verified_user', $is_verified_user)
					->group_by('b.city_id, b.city_name')
					->order_by('b.city_id, b.city_name');
	...
	function get_tutor_list_in_city($city_id, $is_verified_user){
		$this->db->select('b.first_name, b.last_name, b.user_id, b.email_login')
					->from('teacher_open_course_cities a')
					->join('users b', 'a.user_id = b.user_id')
					->where('city_id', $city_id)
					->where('verified_user', $is_verified_user);
