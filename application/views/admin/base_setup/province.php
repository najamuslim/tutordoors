							...
							<tbody>
								...
								<tr>
									...
									<td style="text-align: center">
										<input id="cb-uid-<?php echo $row->province_id;?>" type="checkbox" <?php echo $check_queryable ?> onclick="set_queryable('<?php echo $row->province_id;?>');" />
									</td>
									<td width="40%">
										<?php echo $row->slug ?>
									</td>
...
<script>
	...
	function set_queryable(prov_id){
		if(confirm("Enable this province as queryable will display it on some filtering features. Are you sure?")){
			var setter = $('#cb-uid-'+prov_id).is(':checked') ? "1" : "0";
			$.ajax({
		        type : "POST",
		        url: '<?php echo base_url("location/set_province_queryable");?>',
		        data: "id="+prov_id+"&val="+setter,
		        async: false,
		        dataType: "json",
		        success: function(data) {
		          if(data.status == "200")
