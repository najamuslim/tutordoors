						...
						<table id="default-table" class="table table-bordered table-striped">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<button class="btn btn-warning" onclick="verify_teacher('<?php echo $row->user_id;?>')">Verify this tutor</button>
									</td>
								</tr>
								<?php }	?>	
							</tbody>
...
<script>
	...
	function verify_teacher(id){
		...
		// else{
			$('.replace-id').text(id);
			$('#link-submit').attr('href', "<?php echo base_url('teacher/verify');?>?id="+id);
