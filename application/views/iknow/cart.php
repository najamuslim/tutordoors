            ...
            <table>
              <thead>
                <tr>
                  ...
                  <td>Fee (IDR)</td>
                  <td><?php echo $this->lang->line('fee_per_hour') ?> (IDR)</td>
                  ...
                  if(sizeof($cart_array)==0) 
                    ...
                    foreach($cart_array as $teacher_id => $course_data){
                      ...
                      foreach($course_data as $data){
                        ...
                        $fee_per_hour = $data['pph'];
                        // calculate sub price
                        $sub_price = $fee_per_hour * $data['session_hour'] * $data['class_in_month'] / 1; // unit jam ngajar yang berlaku = 1 jam

                        // echoing output
                        echo '<tr>';
                          echo '<td><strong>'.$teacher_info->first_name.' '.$teacher_info->last_name.'</strong></td>';
                          echo '<td>'.$city.'</td>';
                          ...
                          echo '<td>'.number_format($fee_per_hour, 0, '.', ',').'</td>';
                          echo '<td>'.number_format($data['pph'], 0, '.', ',').'</td>';
