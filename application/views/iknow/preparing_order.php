    ...
    <div class="contant">
        ...
        <div class="row">
          <form id="form-master">
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-12">
                        ...
                        <div class="form-group">
                          ...
                          <div class="checkbox">
                            <label>
                              <input type="checkbox" name="course[]" value="<?php echo $course->course_id; ?>"> <?php echo '<strong>'.$course->program_name.'</strong><br>'.$course->course_name; ?>
                            </label>
                          </div>
                          <?php }} ?>
                        </div>
                      ...
                      <div class="col-md-12">
                        <button id="btn-continue" type="button" class="btn-style" style="float:right;margin-bottom: 8px;" onclick="select_courses()"><?php echo $this->lang->line('continue') ?></button>
                      </div>
                    </div>
                  </fieldset>
                </div>
            ...
            <div class="col-md-8" id="col-form" style="display: none">
              <!-- form courses start -->
              <div id="form-courses">
...
function select_courses(){
  var any_error = false;
  if($('input[name="city"]:checked').val() == undefined){
    any_error = true;
    alert("<?php echo $this->lang->line('please_choose_a_city') ?>");
  }

  var check_course_selected = 0;
  $("input[name='course[]']").each(function(){
    if($(this).is(':checked'))
      check_course_selected += 1;
  })
  if(check_course_selected == 0){
    any_error = true;
    alert("<?php echo $this->lang->line('please_choose_course') ?>");
  }

  if( !any_error ){
    var city_id = $('input[name="city"]:checked').val() == undefined ? "0" : $('input[name="city"]:checked').val();
    $('#col-form').show();
    // 1. clear details
    // $('#form-courses').empty();
    // 2. create summary
    ...
    $("input[name='course[]']").each(function(){
      ...
      if($(this).is(':checked')){
        ...
        course_add_rem('add', course_id, city_id);
        // collecting info
        $.ajax({
          type : "GET",
