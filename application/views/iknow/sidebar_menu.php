                  ...
                  <table class="table">
                    ...
                    <tr>
                      <td>
                        ...
                        <a <?php if($am=="notification") echo 'style="padding-left: 15px;color:#00A685;font-weight:600"';?> href="<?php echo base_url('frontpage/messages'); ?>"><?php echo $this->lang->line('notification') ?> <span class="label label-info">(<?php echo ($unread_message>0 ? $unread_message : 0);?>)</span>
                        </a>
                      </td>
                    </tr>
                    <tr>
...
<script>
  ...
  $("#left-sidebar-province").change(function(e){
    ...
    $.ajax({
      type : "GET",
      url: base_url+'location/get_queryable_cities_by_province/'+$("#left-sidebar-province").val(),
      // data: "id="+$("#ship-city").val(),
      dataType: "json",
      success:function(data){
        $("#left-sidebar-city").find('option').remove().end();
        $("#sel-city").append($("<option></option>").val('all').html(all));
        if(data.status=="200"){
  ...
  $("#left-sidebar-root").change(function(e){
    ...
    $.ajax({
      type : "GET",
      url: base_url+'course/get_course_by_program/'+$('#left-sidebar-root').val(),
      // data: "root="+$("#left-sidebar-root").val(),
      dataType: "json",
      success:function(data){
          $("#left-sidebar-course").find('option').remove().end();
          $("#left-sidebar-course").append($("<option></option>").val('all').html(all));
          for(var i=0; i<data.length;i++)
            $("#left-sidebar-course").append($("<option></option>").val(data[i].id).html(data[i].course_name));
      }
    });
