    	...
    	<div class="container">
        	<div class="row">
            	<div class="col-md-9 col-md-push-3">
                    ...
                    <div class="row">
                    <?php 
                    $check_picture = ($image_thumb=="" ? 0 : 1);										$check_fn = ($user_info->first_name=="" ? 0 : 1);										$check_ln = ($user_info->last_name=="" ? 0 : 1);
                    $check_phone = ($user_info->phone_1=="" ? 0 : 1);										//$check_degree = ($user_info->degree=="" ? 0 : 1);
                    // get open city fo course
                    $check_city = $this->Teacher_m->get_open_city('oc.user_id', $this->session->userdata('userid'));
					$get_latest_edu = $this->User_m->get_education_history_by_userid($this->session->userdata('userid'));					
                    // get open course
                    $check_course = $this->Teacher_m->get_course_data_by_userid($this->session->userdata('userid'));

                    if($check_picture==0 or $check_fn==0 or $check_ln==0 or $check_phone==0 or 					$get_latest_edu->row()->degree==false or $get_latest_edu->row()->major==false					or $get_latest_edu->row()->institution==false or $get_latest_edu->row()->grade_score==false					or $get_latest_edu->row()->date_in==false					or $user_info->verified_user=="0" or $check_city==false or $check_course==false){
                     ?>
                        <div class="col-md-12">
